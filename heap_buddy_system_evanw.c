/*
 * Credits: https://github.com/evanw/buddy-malloc
 *
 ...
 * because we have an 8-byte header and
...
//#define MAX_ALLOC_LOG2 16
//#define MAX_ALLOC_LOG2 24
#define MAX_ALLOC_LOG2 31
#define MAX_ALLOC ((size_t)1 << MAX_ALLOC_LOG2)

/*
 * Allocations are done in powers of two starting
 * from MIN_ALLOC and ending at
 * MAX_ALLOC inclusive. Each allocation size has a
 * bucket that stores the free
 ...
 * "sizeof(list_t)". MIN_ALLOC is currently 16 bytes,
 * so this will be true for
...
/*
 * Each bucket corresponds to a certain allocation size and
 * stores a free list
 ...
 * the entire address space. However, this would cause us to
 * instantly reserve
 ...
 * bit in this array).
 *
 * Given the index for a node, lineraized binary trees allow
 * you to traverse to
 ...
 * information to distinguish between UNUSED and USED from context,
 * so we only
 * need to store SPLIT or not, which only takes a single bit.
 *
 * Note that we don't need to store any nodes for allocations of
 * size MIN_ALLOC
 * since we only ever care about parent nodes.
 */

// ERRORRORORROROOROROOROROOOOROROOR
// TODO: This causes the app to cease working

// static u8int node_is_split[1];
static u8int node_is_split[(1 << (BUCKET_COUNT - 1)) / 8];
// static u8int node_is_split[0x1000000];

// static u32int node_is_split[(1 << (BUCKET_COUNT - 1)) / 8];

/*
 * This is the starting address of the address range for this allocator.
 *
 * Every returned allocation will be an offset of this pointer
 * from 0 to MAX_ALLOC.
 */
static u8int *base_ptr;
 ...
 */
static int update_max_ptr(u8int *new_value) {
...
/*
 * Remove and return the first entry in the list or NULL if the list is
 empty.
 */
 ...
 */
static u8int *ptr_for_node(size_t index, size_t bucket) {
 ...
 */
static size_t node_for_ptr(u8int *ptr, size_t bucket) {
...
/*
 * Given the requested size passed to "malloc", this function returns the
 index
...
void *malloc(size_t request) {
  ...
  if (base_ptr == NULL) {
    ...
    // sbrk(0) returns the current break (= end of heap)
    base_ptr = max_ptr = (u8int *)sbrk(0);
  ...
  while (bucket + 1 != 0) {
     ...
     */
    ptr = (u8int *)list_pop(&buckets[bucket]);
    if (!ptr) {
      /*
...
void free(void *ptr) {
  ...
  i = node_for_ptr((u8int *)ptr, bucket);
